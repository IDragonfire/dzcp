...
switch ($action):
    default:
        ...
        for ($i = 1; $i <= 12; $i++) {
            ...
            $month .= show(_select_field, array(
                "value" => cal($i),
                "sel" => $sel,
                "what" => $mname[$i]
            ));
            ...
            for ($iw = 1; $iw <= 7; $iw++) {
                ...
                if ($wday != $iw) {
                   ...
                   WHERE bday LIKE '" . cal($i) . "." . $monat . ".____" . "'");
                    if (_rows($qry)) {
                        while ($get = _fetch($qry))
                            $infoBday .= jsconvert(_kal_birthday . rawautor($get['id']));
                    ...
                    if (_rows($qry)) {
                        while ($get = _fetch($qry))
                            $infoCW .= jsconvert(_kal_cw . re($get['gegner']));
                   ...
                   WHERE DATE_FORMAT(FROM_UNIXTIME(datum), '%d.%m.%Y') = '" . cal($i) . "." . $monat . "." . $jahr . "'");
                    if (_rows($qry)) {
                        while ($get = _fetch($qry))
                            $infoEvent .= jsconvert(_kal_event . re($get['title']));
    ...
    case 'show';
        ...
        while ($get = _fetch($qry)) {
            ...
            $events .= show($dir . "/event_show", array(
        ...
        $head  = show(_kalender_events_head, array(
            "datum" => date("d.m.Y", $_GET['time'])
        ));
        $index = show($dir . "/event", array(
            "head" => $head,
            "events" => $events
        ));
        break;
