...
switch ($action):
    default:
        if ($chkMe == "unlogged" || $chkMe < 2) {
            ...
            while ($get = _fetch($qry)) {
                if ($get['sparct'] != "")
                    $sparct = show(_taktik_spar_ct, array(
                        "id" => $get['id']
                    ));
                else
                    $sparct = "";
                ...
                if ($get['spart'] != "")
                    $spart = show(_taktik_spar_t, array(
                        "id" => $get['id']
                    ));
                else
                    $spart = "";
                ...
                if ($get['standardct'] != "")
                    $standardct = show(_taktik_standard_ct, array(
                        "id" => $get['id']
                    ));
                else
                    $standardct = "";
                ...
                if ($get['standardt'] != "")
                    $standardt = show(_taktik_standard_t, array(
                        "id" => $get['id']
                    ));
                else
                    $standardt = "";
                ...
                $color++;
                $show .= show($dir . "/taktiken_show", array(
                    "map" => re($get['map']),
                    "id" => $get['id'],
                    "class" => $class,
    ...
    case 'spar':
        if ($chkMe < "2") {
            ...
            $posted = show(_taktik_posted, array(
                "autor" => autor($get['autor']),
                "datum" => date("d.m.Y", $get['datum'])
            ));
            
            $headline = show(_taktik_headline, array(
                "what" => $what,
                "map" => re($get['map'])
            ));
            
            $index = show($dir . "/taktik", array(
                "id" => $_GET['id'],
                "posted" => $posted,
                "headline" => $headline,
                "show" => $show
        ...
        if (!permission("edittactics")) {
            ...
            if ($_GET['what'] == "new") {
                ...
                for ($i = 0; $i < count($files); $i++) {
                    $screen .= show(_member_admin_select_icons, array(
                        "iconimg" => $files[$i]
                    ));
                    $icons = $files[$i];
                }
