...
if ($_POST) {
    if (isset($_GET['ts3']))
        $tsIP = 'ts3server://' . $uip . '/?port=' . $port . '&amp;nickname=' . $nickname . '&amp;channel=' . $channel;
    else
        $tsIP = 'teamspeak://' . $uip . ':' . $port . '/?nickname=' . $nickname . '&amp;loginname=' . $loginname . '&amp;password=' . $password . '&amp;channel=' . $channel . '&amp;channelpassword=' . $channelpassword;
?>
...
?>
  <tr>
    <td class="contentHead" colspan="2"><span class="fontBold">Channel: <?php
    echo rawurldecode($_GET['cName']);
?></span></td>
  </tr>
    <tr>
