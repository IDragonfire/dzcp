        ...
        if (function_exists("fopen")) {
            ...
            while ($get = _fetch($qry)) {
                ...
                if ($get['status'] != "nope" && file_exists(basePath . '/inc/server_query/' . $get['status'] . '.php')) {
                    if (time() - @filemtime(basePath . '/__cache/gameserver_' . intval($get['id']) . '_' . $language . '.html') > $c['cache_server']) {
                        ...
                        if (!$server) {
                            ...
                            if (!file_exists($image_map)) {
                                if ($chkMe == 4)
                                    $index .= '<span style="color:#000;background-color:#FFF"><b style="color:red">Admin:</b> <b>Mappath:</b> ' . str_replace('../', basePath . '/', $image_map) . '<br />';
                        ...
                        $game_icon          = file_exists(basePath . '/inc/images/gameicons/' . $get['status'] . '/' . $server['gamemod'] . '.gif') ? '<img src="../inc/images/gameicons/' . $get['status'] . '/' . $server['gamemod'] . '.gif" alt="" />' : '';
                        ...
                        if (empty($player_list)) {
                            ...
                            if (isset($player_list[1]['score']))
                                $score = 1;
                            else
                                $score = 0;
                            if (isset($player_list[1]['deaths']))
                                $deaths = 1;
                            else
                                $deaths = 0;
                            if (isset($player_list[1]['skill']))
                                $skill = 1;
                            else
                                $skill = 0;
                            if (isset($player_list[1]['goal']))
                                $goal = 1;
                            else
                                $goal = 0;
                            if (isset($player_list[1]['honor']))
                                $honor = 1;
                            else
                                $honor = 0;
                            if (isset($player_list[1]['leader']))
                                $leader = 1;
                            else
                                $leader = 0;
                            if (isset($player_list[1]['ping']))
                                $ping = 1;
                            else
                                $ping = 0;
                            if (isset($player_list[1]['stats']))
                                $stats = 1;
                            else
                                $stats = 0;
                            if (isset($player_list[1]['time']))
                                $time = 1;
                            else
                                $time = 0;
                            
                            if ($score == 1)
                                $showscore = '<td width="60" class="contentHead"><span class="fontBold">Score</span></td>';
                            else
                                $showscore = '';
                            if ($deaths == 1)
                                $showdeaths = '<td width="60" class="contentHead"><span class="fontBold">Deaths</span></td>';
                            else
                                $showdeaths = '';
                            if ($skill == 1)
                                $showskill = '<td width="60" class="contentHead"><span class="fontBold">Skill</span></td>';
                            else
                                $showskill = '';
                            if ($goal == 1)
                                $showgoal = '<td width="60" class="contentHead"><span class="fontBold">Goal</span></td>';
                            else
                                $showgoal = '';
                            if ($honor == 1)
                                $showhonor = '<td width="60" class="contentHead"><span class="fontBold">Honor</span></td>';
                            else
                                $showhonor = '';
                            if ($leader == 1)
                                $showleader = '<td width="60" class="contentHead"><span class="fontBold">Leader</span></td>';
                            else
                                $showleader = '';
                            if ($ping == 1)
                                $showping = '<td width="60" class="contentHead"><span class="fontBold">Ping</span></td>';
                            else
                                $showping = '';
                            if ($stats == 1)
                                $showstats = '<td width="60" class="contentHead"><span class="fontBold">Stats</span></td>';
                            else
                                $showstats = '';
                            if ($time == 1)
                                $showtime = '<td width="90" class="contentHead"><span class="fontBold">' . _server_time . '</span></td>';
                            else
                                $showtime = '';
                            ...
                            foreach ($player_list as $key => $player) {
                                ...
                                $playerstats .= show($dir . "/playerstats", array(
                                    "name" => $player['name'],
                                    "class" => $class,
                                    "id" => $get['id'],
                        ...
                        $klapp = show(_klapptext_server_link, array(
                            "link" => _server_splayerstats,
                            "id" => $get['id'],
                            "moreicon" => $moreicon
                        ));
                        
                        $index .= show($dir . "/server_show", array(
                            "showscore" => $showscore,
                            ...
                            "launch" => strtr($server_link, array(
                                '{IP}' => $get['ip'],
                                '{S_PORT}' => $get['port']
                            )),
                            "port" => $get['port'],
                    ...
                    $index .= show($dir . "/server_show_nope", array(
                        "name" => re($get['name']),
                        "ip" => $get['ip'],
                        "icon" => $gameicon,
                        "pwd" => $pwds,
