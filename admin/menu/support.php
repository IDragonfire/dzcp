...
if(_adminMenu != 'true') exit;
    ...
    function php_5_4_x_version()
    {
		if (!preg_match('/^5\.4(?![.0-9])/', phpversion())) 
		  	return true;
		else
			return false;
    }
    ...
    $support .= "Webserver: ".(array_key_exists('apache2handler', $PhpInfo) ? (array_key_exists('Apache Version', $PhpInfo['apache2handler']) ? $PhpInfo['apache2handler']['Apache Version'] : 'PHP Run as CGI!' ) : 'PHP Run as CGI!')."\r\n";
    $support .= "PHP-Version: ".phpversion()." (".php_sapi_type().")"."\r\n";
    $support .= "MySQL-Server Version: ".(can_use_mysqli() ? mysqli_get_server_info($msql) : mysql_get_server_info())."\r\n";
    $support .= "MySQL-Erweiterung: ".(can_use_mysqli() ? 'MySQLi' : 'MySQL')."\r\n";
    
    if(!can_use_mysqli())
        $support .= "MySQL-Client Version: ".$PhpInfo['mysql']['Client API version']."\r\n";
    else
        $support .= "MySQLi-Client Version: ".$PhpInfo['mysqli']['Client API library version']."\r\n";
    ...
    $support .= "#####################\r\n";    
    $support .= "Memcache: ".$PhpInfo['memcache']['Version']."\r\n";
    
    if(array_key_exists('Zend Data Cache',$PhpInfo))
    {
    	$support .= "Zend-Cache: ".($PhpInfo['Zend Data Cache']['Enabled'] == 'Yes' ? 'On' : 'Off')."\r\n";
    	$support .= "Zend-Cache-Version: ".$PhpInfo['Zend Data Cache']['Version']."\r\n";
    }
    $support .= "\r\n";
    ...
    if(!php_5_4_x_version()) //Removed in PHP 5.4.x
    {
