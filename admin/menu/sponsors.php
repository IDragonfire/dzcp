...
if (!permission("links")) {
    ...
    if ($_GET['do'] == "new") {
        ...
        $show = show($dir . "/form_sponsors", array(
            "head" => _sponsors_admin_head,
            "error" => "",
        ...
        if (empty($_POST['name']) || empty($_POST['link']) || empty($_POST['beschreibung'])) {
            if (empty($_POST['beschreibung']))
                $error = show("errors/errortable", array(
                    "error" => _sponsors_empty_beschreibung
                ));
            if (empty($_POST['link']))
                $error = show("errors/errortable", array(
                    "error" => _sponsors_empty_link
                ));
            if (empty($_POST['name']))
                $error = show("errors/errortable", array(
                    "error" => _sponsors_empty_name
                ));
        ...
        while ($get = _fetch($qry)) {
            ...
            $show .= show($dir . "/sponsors_show", array(
                "link" => cut(re($get['link']), 40),
                "class" => $class,
                "name" => $get['name'],
                "edit" => $edit,
                "delete" => $delete
