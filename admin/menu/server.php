...
if (!permission("editserver")) {
    ...
    while ($get = _fetch($qry)) {
        ...
        if ($get['navi'] == "0")
            $menu = show(_server_menu_icon_yes, array(
                "id" => $get['id']
            ));
        else
            $menu = show(_server_menu_icon_no, array(
                "id" => $get['id']
            ));
        
        $show_ .= show($dir . "/server_show", array(
            "gameicon" => $gameicon,
    ...
    if ($_GET['do'] == "ts") {
        ...
        $show = show($dir . "/server_edit", array(
            "edithead" => _admin_server_edit,
            "ip" => _server_ip,
            "sip" => re($get['ip']),
            "name" => _server_name,
            "sname" => re($get['name']),
            ...
            "ftp_host" => re($get['ftp_host']),
            "ftp_login" => re($get['ftp_login']),
            "value" => _button_value_edit,
            "ftp_pwd" => re($get['ftp_pwd']),
            "ftp_pfad" => re($get['bl_path']),
            "ftp_file" => re($get['bl_file']),
        ...
        if (empty($_POST['ip']) || empty($_POST['port'])) {
            ...
            if ($_POST['game'] == "lazy")
                ...
                $game = "`game` = '" . up($_POST['game']) . "',";
            
            if ($_POST['status'] == "lazy")
                $status = "";
            else
        ...
        $show = show($dir . "/server_add", array(
            "newhead" => _admin_server_new,
            "ip" => _server_ip,
            "pwd" => _pwd,
