...
if ($chkMe != 4) {
    ...
    if ($_GET['do'] == "add") {
        ...
        for ($i = 0; $i < count($files); $i++) {
            $banners .= show(_partners_select_icons, array(
                "icon" => $files[$i],
                "sel" => ""
            ));
        }
        $show = show($dir . "/form_partners", array(
            "do" => "addbutton",
        ...
        while ($get = _fetch($qry)) {
            ...
            $color++;
            $show_ .= show($dir . "/partners_show", array(
                "class" => $class,
