...
if (permission("news")) {
    ...
    if ($_GET['do'] == "add") {
        ...
        $timeshift_date = show(_dropdown_date_ts, array(
            "nr" => "ts",
            "day" => dropdown("day", date("d", time())),
            "month" => dropdown("month", date("m", time())),
            "year" => dropdown("year", date("Y", time()))
        ...
        $timeshift_time = show(_dropdown_time_ts, array(
            "nr" => "ts",
            "hour" => dropdown("hour", date("H", time())),
            "minute" => dropdown("minute", date("i", time())),
            "uhr" => _uhr
        ...
        $show = show($dir . "/news_form", array(
            "head" => _admin_news_head,
            "nautor" => _autor,
            "autor" => autor($userid),
        ...
        while ($get = _fetch($qry)) {
            ...
            $titel  = show(_news_show_link, array(
                "titel" => re(cut($get['titel'], $lnewsadmin)),
                "id" => $get['id']
            ));
            ...
            $public = ($get['public'] == 1) ? '<a href="?admin=newsadmin&amp;do=public&amp;id=' . $get['id'] . '&amp;what=unset"><img src="../inc/images/public.gif" alt="" title="' . _non_public . '" /></a>' : '<a href="?admin=newsadmin&amp;do=public&amp;id=' . $get['id'] . '&amp;what=set"><img src="../inc/images/nonpublic.gif" alt="" title="' . _public . '" /></a>';
            if (empty($get['datum']))
                $datum = _no_public;
            else
                $datum = date("d.m.y H:i", $get['datum']) . _uhr;
            
            $show_ .= show($dir . "/admin_show", array(
                "date" => $datum,
                "titel" => $titel,
                "class" => $class,
                "autor" => autor($get['autor']),
        ...
        $show = show($dir . "/admin_news", array(
            "head" => _news_admin_head,
            "nav" => $nav,
            "autor" => _autor,
            "titel" => _titel,
            "val" => "newsadmin",
