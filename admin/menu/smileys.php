...
if ($chkMe != 4) {
    ...
    if ($_GET['do'] == "add") {
        $show = show($dir . "/form_smileys", array(
            "head" => _smileys_head_add,
        ...
        if (!$tmpname || empty($_POST['bbcode']) || $type == "image/pjpeg" || $type == "image/jpeg" || !$imageinfo[0] || preg_match("#[[:punct:]]|[[:space:]]#", $_POST['bbcode']) || file_exists($spfad . $_POST['bbcode'] . ".gif")) {
            ...
            elseif (preg_match("#[[:punct:]]|[[:space:]]#", $_POST['bbcode']))
                $show = error(_smileys_specialchar, 1);
            elseif (file_exists($spfad . $_POST['bbcode'] . ".gif"))
        ...
        $show = show($dir . "/form_smileys_edit", array(
            "head" => _smileys_head_edit,
            "bbcode" => _smileys_bbcode,
            "id" => $_GET['id'],
        ...
        for ($i = 0; $i < count($files); $i++) {
            if ($files[$i] != '^^.gif') {
                ...
                $show_ .= show($dir . "/smileys_show", array(
                    "bbcode" => $bbc,
                    "smiley" => $smileys,
                    "class" => $class,
                    "del" => $delete,
