...
if (!permission("serverliste")) {
    ...
    while ($get = _fetch($qry)) {
        ...
        if (empty($get['clanurl'])) {
            $clanname = show(_slist_clanname_without_url, array(
                "name" => re($get['clanname'])
            ));
        } else {
            $clanname = show(_slist_clanname_with_url, array(
                "name" => re($get['clanname']),
                "url" => re($get['clanurl'])
            ));
        }
        ...
        $color++;
        $show_ .= show($dir . "/slist_show", array(
            "id" => $get['id'],
            "clanname" => $clanname,
            "serverip" => re($get['ip']),
