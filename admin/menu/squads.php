...
if (!permission("editsquads")) {
    ...
    while ($get = _fetch($qry)) {
        ...
        $color++;
        $squads .= show($dir . "/squads_show", array(
            "squad" => '<a href="../squads/?action=shows&amp;id=' . $get['id'] . '" style="display:block">' . re($get['name']) . '</a>',
            "game" => re($get['game']),
            "icon" => $icon,
            "edit" => $edit,
            "class" => $class,
    ...
    $show = show($dir . "/squads", array(
        "memberadminheader" => _member_admin_header,
    ...
    if ($_GET['do'] == "add") {
        ...
        for ($i = 0; $i < count($files); $i++) {
            if (preg_match("=\.gif|.jpg|.png=Uis", $files[$i]))
                $gameicons .= show(_select_field, array(
                    "value" => $files[$i],
        ...
        $show = show($dir . "/squads_add", array(
            "memberadminaddheader" => _member_admin_add_header,
            "squad" => _member_admin_squad,
            "pos" => _position,
        ...
        $show = show($dir . "/squads_edit", array(
            "memberadminaddheader" => _member_admin_edit_header,
            "squad" => _member_admin_squad,
            "id" => intval($_GET['id']),
            "pos" => _position,
            "icon" => _member_admin_icon,
