    ...
    if ($_GET['show'] == "subkats") {
        ...
        while ($getk = _fetch($qryk)) {
            if (!empty($getk['kattopic'])) {
                $subkat = show(_config_forum_subkats, array(
                    "topic" => re($getk['kattopic']),
                ...
                $color++;
                $subkats .= show($dir . "/forum_show_subkats_show", array(
                    "subkat" => $subkat,
                    "delete" => $delete,
                    "class" => $class,
                    "edit" => $edit
            ...
            $skathead = show(_config_forum_subkathead, array(
                "kat" => re($getk['name'])
            ));
            $add      = show(_config_forum_subkats_add, array(
                "id" => $_GET['id']
            ));
            
            $show = show($dir . "/forum_show_subkats", array(
                "head" => _config_forum_head,
                "subkathead" => $skathead,
                "subkats" => $subkats,
                "add" => $add,
        ...
        while ($get = _fetch($qry)) {
            ...
            $class = ($color % 2) ? "contentMainSecond" : "contentMainFirst";
            $color++;
            $kats .= show($dir . "/forum_show_kats", array(
                "class" => $class,
                "kat" => $kat,
                "status" => $status,
        ...
        if ($_GET['do'] == "newkat") {
            ...
            $show = show($dir . "/katform", array(
                "fkat" => _config_katname,
            ...
            while ($get = _fetch($qry)) {
                ...
                $show = show($dir . "/katform_edit", array(
                    "fkat" => _config_katname,
            ...
            $qry = db("SELECT * FROM " . $db['f_skats'] . " WHERE sid = " . (int) $_GET['id'] . " ORDER BY pos");
            while ($get = _fetch($qry)) {
                $positions .= show(_select_field, array(
                    "value" => $get['pos'] + 1,
            ...
            $show = show($dir . "/skatform", array(
                "head" => _config_forum_add_skat,
