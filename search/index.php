...
switch ($action):
    default:
            ...
            if ($_GET['si_board'] == true) {
                    ...
                    if ($_GET['con'] == 'or') {
                        ...
                        for ($x = 0; $x < count($suche); $x++) {
                            ...
                            if ($_GET['area'] != 'topic')
                                ...
                                $dosearch .= $c . " s1.topic LIKE '%" . trim($suche[$x]) . "%' ";
                        }
                    } else {
                        if ($_GET['area'] != 'topic')
                            $dosearch .= "WHERE (s1.t_text LIKE '%" . trim($_GET['search']) . "%' OR s2.text LIKE '%" . trim($_GET['search']) . "%'";
                        else
                            $dosearch .= "WHERE (s1.topic LIKE '%" . trim($_GET['search']) . "%'";
                    }
                    $dosearch .= ')';
                }
                ...
                while ($get = _fetch($qry)) {
                    $intF = db("SELECT * FROM " . $db['f_access'] . "
                      WHERE user = '" . $_SESSION['id'] . "'
                      AND forum = '" . $get['subid'] . "'");
                    if (($get['intern'] == 1 && !_rows($intF) && $chkMe != 4))
                        $entrys--;
                    if ($get['intern'] == 0 || (($get['intern'] == 1 && _rows($intF)) || $chkMe == 4)) {
                        if ($get['sticky'] == 1)
                        ...
                        $threadlink = show(_forum_thread_search_link, array(
                            "topic" => cut(re($get['topic']), $lforumtopic),
                            "id" => $get['id'],
                            "sticky" => $sticky,
                            "hl" => $_GET['search'],
                            "closed" => $closed,
                        ...
                        $results .= show($dir . "/forum_search_results", array(
                ...
                $show = show($dir . "/forum_search_show", array(
                    "head" => _forum_search_results,
                    "autor" => _autor,
        ...
        $index = show($dir . "/search", array(
            "head" => /*_search_head*/ _forum_search_head,
    ...
    case 'site';
        ...
        while ($get = _fetch($qry)) {
            ...
            $color++;
            $shownews .= show($dir . "/search_show", array(
                "class" => $class,
                "type" => 'news',
        ...
        $index = show($dir . "/search_global", array(
            "shownews" => $shownews,
