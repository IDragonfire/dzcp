...
function server_query_halflife($ip, $port, $q_port, $request)
{
    ...
    if ($request == "players") {
        ...
        do {
            ...
            $player[$playernumber]['score'] = (ord($buffer[$position + 1])) + (ord($buffer[$position + 2]) * 256) + (ord($buffer[$position + 3]) * 65536) + (ord($buffer[$position + 4]) * 16777216);
