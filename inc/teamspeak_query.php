    ...
    for ($i = 0; $i < count($pcArr); $i++) {
        ...
        for ($j = 0; $j < count($ccArr); $j++) {
            ...
            if ($innerArr[0] == $innerCCArray[2]) {
                ...
                $subchan .= show("menu/teamspeak_subchan", array(
        ...
        $chan .= show("menu/teamspeak_chan", array(
            "channel" => $channels,
            "subchannels" => $subchan
        ));
        ...
        $teamspeak = show("menu/teamspeak", array(
            "hostname" => $hostname,
            "channels" => $chan
        ));
    }
...
class TSStatus {
    ...
    function sendCommand($fp, $cmd) {
        ...
        if (!empty($response) && !strstr($response, 'error id=0')) {
            $this->error = strtr(rep2($response), array(
                ' msg=' => '<br />msg=',
                ' extra_msg=' => '<br />extra_msg='
            ));
        }
    ...
    function tsvars($str) {
        ...
        for ($i = 0; $i < sizeof($str); $i++)
            $vars[trim(array_shift(explode('=', $str[$i], 2)))] = trim(array_pop(explode('=', $str[$i], 2)));
        return $vars;
    }
