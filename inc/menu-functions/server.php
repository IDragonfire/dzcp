    ...
    if (empty($serverID)) {
      ...
      ';
        ...
        if (!empty($server_name_config[$server['gamemod']]))
            $server_name_short = $server_name_config[$server['gamemod']][1];
        ...
        $servernavi .= show("menu/server", array(
            "host" => re(cut($server['hostname'], $servermenu)),
            "ip" => $get['ip'],
