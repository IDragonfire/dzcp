...
function server($serverID = 0)
{
    ...
    else
    {
        ...
        if(cache('nav_server_'.$serverID, config('cache_server'), 'c'))
        {
            ...
            if($server_online)
            {
                ...
                if(!empty($player_list) && count($player_list) >= 1)
                {
                    ...
                    { if(array_key_exists('name',$player)) $players .= str_replace("'", '', htmlentities($player['name'])) . ', '; }
                }
            }
            ...
            cache('nav_server_'.$serverID, $servernavi, 'w');
        }
        else
            $servernavi = cache('nav_server_'.$serverID, null, 'r');
    }
