...
function shout($ajax = 0) {
    ...
    while ($get = _fetch($qry)) {
        ...
        $show .= show("menu/shout_part", array(
            "nick" => $nick,
    ...
    if (settings('reg_shout') == 1 && $chkMe == 'unlogged') {
        ...
        if ($chkMe == "unlogged") {
            $form = show("menu/shout_form", array(
                "dis" => $dis
            ));
            $sec  = show("menu/shout_antispam", array(
