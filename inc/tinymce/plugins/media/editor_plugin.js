	...
	tinymce.create('tinymce.plugins.MediaPlugin', {
		init : function(ed, url) {
			...
			function isMediaElm(n) {
				return /^(Flash|DZCP|YouTube)$/.test(n.className);
			};
		...
		getInfo : function() {
			return {
				...
				infourl : 'http://wiki.moxiecode.com//TinyMCE:Plugins/media',
		...
		_objectsToSpans : function(ed, o) {
			...
			h = h.replace(/<script[^>]*>\s*write(Flash|DZCP|YouTube)\(\{([^\)]*)\}\);\s*<\/script>/gi, function(a, b, c) {
				var o = t._parse(c);
