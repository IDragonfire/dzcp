...
function replace($txt, $type = 0, $no_vid_tag = 0) {
    ...
    $txt = preg_replace_callback("#\<img(.*?)\>#", create_function('$img', 'if(preg_match("#class#i",$img[1]))
                                    return "<img".$img[1].">";
...
function smileys($txt) {
    ...
    $repl = array(
        " <img src=\"../inc/images/smileys/grin.gif\" alt=\"\" />",
...
function flagge($txt) {
    ...
    $repl = array(
        "<img src=\"../inc/images/flaggen/de.gif\" alt=\"\" />",
...
function highlight($word) {
    if (substr(phpversion(), 0, 1) == 5)
        $word = str_ireplace($word, '<span class="fontRed">' . $word . '</span>', $word);
    else
        $word = str_replace($word, '<span class="fontRed">' . $word . '</span>', $word);
...
function check_msg() {
    ...
    if (_rows($qry)) {
        $upd = db("UPDATE " . $db['msg'] . "
                 SET `page` = '1'
                 WHERE an = '" . $_SESSION['id'] . "'");
        return show("user/new_msg", array(
            "new" => _site_msg_new
        ));
    }
}
...
function cw_result_nopic($punkte, $gpunkte) {
    if ($punkte > $gpunkte)
        $result = '<span class="CwWon">' . $punkte . ':' . $gpunkte . '</span>';
    elseif ($punkte < $gpunkte)
        $result = '<span class="CwLost">' . $punkte . ':' . $gpunkte . '</span>';
    else
        $result = '<span class="CwDraw">' . $punkte . ':' . $gpunkte . '</span>';
...
function set_cookie($name, $value = '', $path = '/', $secure = false, $http_only = true) {
    ...
    header('Set-Cookie: ' . rawurlencode($name) . '=' . rawurlencode($value) . (empty($expires) ? '' : '; expires=' . gmdate('D, d-M-Y H:i:s \\G\\M\\T', $expires)) . (empty($path) ? '' : '; path=' . $path) . '; domain=' . $domain . (!$secure ? '' : '; secure') . (!$http_only ? '' : '; HttpOnly'), false);
}
//-> Passwortabfrage
...
function error($error, $back = 1) {
    return show("errors/error", array(
        "error" => $error,
        "back" => $back,
        "fehler" => _error,
...
function error2($error) {
    return show("errors/error2", array(
        "error" => $error,
        "fehler" => _error
    ));
}
...
function artikelSites($sites, $id) {
    ...
    for ($i = 0; $i < $sites; $i++) {
        if ($i == $part) {
            $seiten .= show(_page, array(
                "num" => ($i + 1)
            ));
        } else {
            $seiten .= show(_artike_sites, array(
                "part" => $i,
                "id" => $id,
                "num" => ($i + 1)
            ));
...
function autor($uid, $class = "", $nick = "", $email = "", $cut = "", $add = "") {
    ...
    if (_rows($qry)) {
        ...
        $result   = show(_user_link_noreg, array(
            "nick" => $nickname,
            "class" => $class,
...
function cleanautor($uid, $class = "", $nick = "", $email = "", $cut = "") {
    ...
    $get = _fetch($qry);
    if (_rows($qry)) {
        $result = show(_user_link_preview, array(
            "id" => $uid,
            "country" => flag($get['country']),
            "class" => $class,
            "nick" => re($get['nick'])
...
function fabo_autor($uid) {
    ...
    $get = _fetch($qry);
    if (_rows($qry))
        $result = show(_user_link_fabo, array(
            "id" => $uid,
            "nick" => re($get['nick'])
        ));
...
function blank_autor($uid) {
    ...
    $get = _fetch($qry);
    if (_rows($qry))
        $result = show(_user_link_blank, array(
            "id" => $uid,
            "nick" => re($get['nick'])
        ));
...
function check_msg_emal() {
    ...
    while ($get = _fetch($qry)) {
        if ($get['pnmail'] == 1) {
            ...
            $subj = show(settings('eml_pn_subj'), array(
                "domain" => $httphost
            ));
            
            $message = show(settings('eml_pn'), array(
                "nick" => re($get['nick']),
                "domain" => $httphost,
                "titel" => $get['titel'],
                "clan" => $clanname
...
function infos($checkBrowser = "") {
    ...
    if (settings("persinfo") == 1) {
        ...
        if (preg_match("/Android/i", $data))
            $system = "Android";
        elseif (preg_match("/Linux/i", $data))
            $system = "Linux";
        elseif (preg_match("/SunOS/i", $data))
            $system = "Sun OS";
        elseif (preg_match("/Macintosh/i", $data))
            $system = "Macintosh";
        elseif (preg_match("/Mac_PowerPC/i", $data))
            $system = "Macintosh";
        elseif (preg_match("/Windows 2000/i", $data))
            $system = "Windows 2000";
        elseif (preg_match("/Windows XP/i", $data))
            $system = "Windows XP";
        elseif (preg_match("/NT 5.2/i", $data))
            $system = "Windows XP x64";
        elseif (preg_match("/NT 5.1/i", $data))
            $system = "Windows XP";
        elseif (preg_match("/NT 5.0/i", $data))
            $system = "Windows 2000";
        elseif (preg_match("/NT 4.0/i", $data))
            $system = "Windows NT 4";
        elseif (preg_match("/NT 6.0/i", $data))
            $system = "Windows Vista";
        elseif (preg_match("/NT 6.1/i", $data))
            $system = "Windows 7";
        elseif (preg_match("/Windows ME/i", $data))
            $system = "Windows 9x + ME";
        elseif (preg_match("/Windows 98/i", $data))
            $system = "Windows 9x + ME";
        elseif (preg_match("/Windows 95/i", $data))
            $system = "Windows 9x + ME";
        elseif (preg_match("/Win 9x/i", $data))
            $system = "Windows 9x + ME";
        elseif (preg_match("/Win95/i", $data))
            $system = "Windows 9x + ME";
        elseif (preg_match("/Win98/i", $data))
            $system = "Windows 9x + ME";
        elseif (preg_match("/OS (.*?) like Mac OS X/i", $data))
            $system = "iOS";
        else
            $system = _unknown_system;
        
        if (preg_match("/Opera/i", $data))
            $browser = "Opera";
        elseif (preg_match("/Konqueror/i", $data))
            $browser = "Konqueror";
        ...
        elseif (preg_match("/Safari/i", $data))
            $browser = "Safari";
        elseif (preg_match("/MSIE 5/i", $data))
            $browser = "Internet Explorer 5";
        elseif (preg_match("/MSIE 6/i", $data))
            $browser = "Internet Explorer 6";
        elseif (preg_match("/MSIE 7/i", $data))
            $browser = "Internet Explorer 7";
        elseif (preg_match("/MSIE 8/i", $data))
            $browser = "Internet Explorer 8";
        elseif (preg_match("/MSIE 9/i", $data))
            $browser = "Internet Explorer 9";
        elseif (preg_match("/MSIE 10/i", $data))
            $browser = "Internet Explorer 10";
        else
            $browser = _unknown_browser;
        
        $res = "<script language=\"javascript\" type=\"text/javascript\">
                  doc.write(screen.width + ' x ' + screen.height)
              </script>";
        
        $infos = show("menu/pers.infos", array(
            "ip" => $userip,
            "info_ip" => _info_ip,
...
//-->
    ...
    if (settings("wmodus") && $chkMe != 4) {
        ...
        $login = show("errors/wmodus_login", array(
            "what" => _login_login,
            "secure" => $secure,
        ...
        echo show("errors/wmodus", array(
            "wmodus" => _wartungsmodus,
        ...
        for ($i = 0; $i < count($tmps); $i++) {
            ...
            $tmpldir .= show(_select_field, array(
                "value" => "../user/?action=switch&amp;set=" . $tmps[$i],
                "what" => $tmps[$i],
                "sel" => $selt
            ));
        }
        ...
        //misc vars
        $template_switch = show("menu/tmp_switch", array(
            "templates" => $tmpldir
        ));
        $clanname        = re(settings("clanname"));
        $time            = show(_generated_time, array(
            "time" => $time
        ));
        $headtitle       = show(_index_headtitle, array(
            "clanname" => $clanname
        ));
        $rss             = $clanname;
        $dir             = $designpath;
