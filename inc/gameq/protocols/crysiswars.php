...
class GameQ_Protocols_Crysiswars extends GameQ_Protocols_Gamespy3
{
    ...
    protected $name_short = "Crysis Wars";
    protected $basic_game_dir = 'game';
     ...
     * @var array
    */
    protected $settings_filter = array('timelimit','official','voicecomm','friendlyfire','dx10','gamepadsonly','timeleft');
    ...
    */
    protected function process_dzcp_runtime()
    {
        ...
        //Hack
        $this->server_data_stream['teams'] = array(0 => array('team' => 'NORDKOREA', 'score' => ''), 1 => array('team' => 'USA', 'score' => ''), 2 => array('team' => 'Spectator', 'score' => ''));

        $mod_name_long = ''; $mod_name_short = ''; $is_mod_ml = false;
        if(!$this->is_mod && count($this->modlist) && array_key_exists($this->server_data_stream['gamemode'], $this->modlist))
        {
            $mod_name_long = $this->modlist[$this->server_data_stream['gamemode']]['name_long'];
            $mod_name_short = $this->modlist[$this->server_data_stream['gamemode']]['name_short'];
            $is_mod_ml = true;
        }

        if(!$this->is_mod && !$is_mod_ml && $this->basic_game_dir != $this->server_data_stream['gamemode'])
        {
            DebugConsole::insert_info('GameQ_Protocols_Crysis::process_dzcp_runtime()', 'The basic-gamedir differs from servers gamedir, use a mod?');
            DebugConsole::insert_info('GameQ_Protocols_Crysis::process_dzcp_runtime()', 'Basic: "'.$this->basic_game_dir.'" <=> Server: "'.$this->server_data_stream['gamemode'].'" on Server '.$this->server_data_stream['gq_address'].':'.$this->server_data_stream['gq_port']);
            $result->add('game_use_mod', true);
            $result->add('game_mod_dir', $this->server_data_stream['gamemode']);
        }
        else
            $result->add('game_use_mod', false);

        $secure = array('enable' => $this->server_data_stream['anticheat'] == '1' ? true : false, 'pic' => 'punkbuster', 'name' => 'PunkBuster');
        ...
        $result->add('game_map_pic_dir', 'gamespy3/cryengine/crysiswars'.($this->is_mod || $is_mod_ml ? '/'.$this->server_data_stream['gamemode'] : '') );
        $result->add('game_type',ucfirst($this->server_data_stream['gametype']));
        $result->add('game_dir', !$this->is_mod && !$is_mod_ml ? 'crysiswars' : $this->server_data_stream['gamemode']);
        $result->add('game_mod', $this->is_mod || $is_mod_ml ? $this->server_data_stream['gamemode'] : '');
        ...
        $result->add('game_engine', 'cryengine');
        ...
        if($this->server_data_stream['gq_online'] == '1')
            GameQ::mkdir_img('maps/gamespy3/cryengine/crysiswars/'.($this->is_mod || $is_mod_ml ? $this->server_data_stream['gamemode'] : $this->basic_game_dir));
        ...
        */
        ...
        if(array_key_exists('players', $this->server_data_stream) && count($this->server_data_stream['players']) >= 1)
        {
            foreach($this->server_data_stream['players'] as $player)
            {
                ...
                $new_player['player_team'] = (string)(empty($player['team']) ? '3' : $player['team']);
                $new_player['player_squad'] = '0';
                $new_player['player_kills'] = (string)(empty($player['kills']) ? '0' : $player['kills']);
                $new_player['player_deaths'] = (string)(empty($player['deaths']) ? '0' : $player['deaths']);
                $new_player['player_rank'] = (string)(empty($player['rank']) ? '0' : $player['rank']);
