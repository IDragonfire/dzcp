 ...
 *
 *
 * Modified by Godkiller_NT (Hammermaps.de) for DZCP-Extended Edition 03-10-2013 (mm-dd-yyyy)
 *
 *
 */
class GameQ_Protocols_Teamspeak3 extends GameQ_Protocols
{
    ...
    protected $packets = array(
            ...
            self::PACKET_PLAYERS => "use port=%d\x0Aclientlist -uid -times -away -voice -groups -info -icon -country\x0A",
            self::PACKET_CHANNELS => "use port=%d\x0Achannellist -topic -flags -voice -limits -icon\x0A",
            self::PACKET_SGROUPLIST => "use port=%d\x0Aservergrouplist\x0A",
            self::PACKET_CGROUPLIST => "use port=%d\x0Achannelgrouplist\x0A"
    );
    ...
    /**
     * Process the Server Grouplist information
     */
    protected function process_sgrouplist()
    {
        // Make sure we have a valid response
        if(!$this->hasValidResponse(self::PACKET_SGROUPLIST))
            return array();

        // Let's preprocess the status
        $buffer = $this->preProcess($this->packets_response[self::PACKET_SGROUPLIST]);
        ...
        { $data_to_g[$array['sgid']] = $array; }

        // Set the result to a new result instance
        $result = new GameQ_Result();

        // Loop the data and add it to the result
        foreach($data_to_g AS $key => $value)
        { $result->addSgrouplist($key, $value); }

        unset($data, $data_to_g, $buffer, $key, $value);
        return $result->fetch();
    }

    /**
     * Process the Channel Grouplist information
     */
    protected function process_cgrouplist()
    {
        // Make sure we have a valid response
        if(!$this->hasValidResponse(self::PACKET_CGROUPLIST))
            return array();

        // Let's preprocess the status
        $buffer = $this->preProcess($this->packets_response[self::PACKET_CGROUPLIST]);
        ...
        foreach($data_to_g AS $key => $value)
        {
            $result->addCgrouplist($key, $value);
        }
