...
class GameQ_Protocols_Crysis2 extends GameQ_Protocols_Gamespy3
{
    ...
    protected $name_short = "Crysis 2";
    protected $basic_game_dir = 'crysis2';
    ...
    */
    protected function process_dzcp_runtime()
    {
        ...
        if(!$this->is_mod && count($this->modlist) && array_key_exists($this->server_data_stream['modname'], $this->modlist))
        {
        ...
        //Hack
        $this->server_data_stream['modname'] = ($this->server_data_stream['modname'] == 'NoMod' ? 'crysis2' : $this->server_data_stream['modname']);
        if(!$this->is_mod && !$is_mod_ml && $this->basic_game_dir != $this->server_data_stream['modname'])
        {
            DebugConsole::insert_info('GameQ_Protocols_Crysis2::process_dzcp_runtime()', 'The basic-gamedir differs from servers gamedir, use a mod?');
            DebugConsole::insert_info('GameQ_Protocols_Crysis2::process_dzcp_runtime()', 'Basic: "'.$this->basic_game_dir.'" <=> Server: "'.$this->server_data_stream['modname'].'" on Server '.$this->server_data_stream['gq_address'].':'.$this->server_data_stream['gq_port']);
        ...
        $result->add('game_map_pic_dir', 'gamespy3/cryengine/crysis2'.($this->is_mod || $is_mod_ml ? '/'.$this->server_data_stream['modname'] : '/'.'crysis2') );
        $result->add('game_type',ucfirst($this->server_data_stream['gametype']));
        $result->add('game_dir', !$this->is_mod && !$is_mod_ml ? 'crysis2' : $this->server_data_stream['modname']);
        $result->add('game_mod', $this->is_mod || $is_mod_ml ? $this->server_data_stream['modname'] : 'crysis2');
        ...
        if($this->server_data_stream['gq_online'] == '1')
            GameQ::mkdir_img('maps/gamespy3/cryengine/crysis2/'.($this->is_mod || $is_mod_ml ? $this->server_data_stream['modname'] : $this->basic_game_dir));
