...
class GameQ_Protocols_Fear extends GameQ_Protocols_Gamespy2
{
    ...
    */
    protected function process_dzcp_runtime()
    {
        ...
        if(!$this->is_mod && count($this->modlist) && array_key_exists($this->server_data_stream['gsgamename'], $this->modlist))
        {
            $mod_name_long = $this->modlist[$this->server_data_stream['gsgamename']]['name_long'];
            $mod_name_short = $this->modlist[$this->server_data_stream['gsgamename']]['name_short'];
            $is_mod_ml = true;
        }

        if(!$this->is_mod && !$is_mod_ml && $this->basic_game_dir != $this->server_data_stream['gsgamename'])
        {
            DebugConsole::insert_info('GameQ_Protocols_Fear::process_dzcp_runtime()', 'The basic-gamedir differs from servers gamedir, use a mod?');
            DebugConsole::insert_info('GameQ_Protocols_Fear::process_dzcp_runtime()', 'Basic: "'.$this->basic_game_dir.'" <=> Server: "'.$this->server_data_stream['gsgamename'].'" on Server '.$this->server_data_stream['gq_address'].':'.$this->server_data_stream['gq_port']);
            $result->add('game_use_mod', true);
            $result->add('game_mod_dir', $this->server_data_stream['gsgamename']);
        }
        else
        ...
        $result->add('game_mod', $this->is_mod || $is_mod_ml ? $this->server_data_stream['gsgamename'] : '');
        $result->add('game_country','');
        $result->add('game_region','');
        $result->add('game_os', $this->server_data_stream['linux'] ? 'linux' : 'windows');
        $result->add('game_dedicated', $this->server_data_stream['dedicated'] ? true : false);
        ...
        $result->add('game_engine', 'lithtech');
        ...
        if($this->server_data_stream['gq_online'] == '1')
            GameQ::mkdir_img('maps/lithtech/'.$this->basic_game_dir.'/'.($this->is_mod || $is_mod_ml ? $this->server_data_stream['gsgamename'] : $this->basic_game_dir));
