...
switch ($action):
    default:
        ...
        $anz_ges_points = show(_cw_stats_ges_points, array(
            "ges_won" => $sum_punkte,
        ...
        if (cnt($db['cw'], " WHERE datum < " . time() . "") != "0") {
            ...
            if ($anz_wo_wars != "0")
                $wo_balken = show(_votes_balken, array(
                    "width" => $wo_rawpercent
                ));
            else
                $wo_balken = show(_votes_balken, array(
            ...
            if ($anz_lo_wars != "0")
                $lo_balken = show(_votes_balken, array(
                    "width" => $lo_rawpercent
                ));
            else
                $lo_balken = show(_votes_balken, array(
            ...
            if ($anz_dr_wars != "0")
                $dr_balken = show(_votes_balken, array(
                    "width" => $dr_rawpercent
                ));
            else
                $dr_balken = show(_votes_balken, array(
        ...
        $anz_ges_wars = show(_cw_stats_ges_wars, array(
            "ge_wars" => $anz_ge_wars
        ));
        
        $stats_all = show($dir . "/stats", array(
            "wo_wars" => $anz_wo_wars,
            "lo_wars" => $anz_lo_wars,
            "dr_wars" => $anz_dr_wars,
        ...
        while ($get = _fetch($qry)) {
            if ($_GET['showsquad'] == $get['id'] || $_GET['show'] == $get['id']) {
                $shown   = show(_klapptext_show, array(
                    "id" => $get['id']
                ));
                $display = "";
            } else {
                $shown   = show(_klapptext_dont_show, array(
                    "id" => $get['id']
                ));
                $display = "none";
            }
            
            $img  = show(_gameicon, array(
                "icon" => $get['icon']
            ));
            $wars = "";
            ...
            while ($getm = _fetch($qrym)) {
                ...
                $gegner = show(_cw_details_gegner, array(
                    "gegner" => re(cut($getm['clantag'] . " - " . $getm['gegner'], $lcwgegner)),
                    "url" => '?action=details&amp;id=' . $getm['id']
                ));
                
                $details = show(_cw_show_details, array(
                    "id" => $getm['id']
                ));
                $squad   = show(_member_squad_squadlink, array(
                    "squad" => re($get['name']),
                    "id" => $get['id'],
                    "shown" => $shown
                ));
                
                $wars .= show($dir . "/clanwars_show2", array(
                    "datum" => date("d.m.Y", $getm['datum']),
                    "img" => $img,
                    "flagge" => $flagge,
                    "gegner" => $gegner,
                    "xonx" => re($getm['xonx']),
            ...
            $anz_ges_wars = show(_cw_stats_ges_wars_sq, array(
                "ge_wars" => $anz_ge_wars
            ));
            
            $stats = show($dir . "/stats", array(
                "wo_wars" => $anz_wo_wars,
                "lo_wars" => $anz_lo_wars,
                "dr_wars" => $anz_dr_wars,
                "dr_percent" => $dr_percent,
            ...
            if (cnt($db['cw'], " WHERE squad_id = " . $get['id'] . " AND datum < " . time() . "") > 0) {
                $show .= show($dir . "/squads_show", array(
                    "id" => $get['id'],
                    "shown" => $shown,
                    "display" => $display,
                    "wars" => $wars,
        ...
        while ($get = _fetch($qry)) {
            ...
            $img = squad($get['icon']);
            $legende .= show(_cw_legende, array(
                "game" => re($get['game']),
                "img" => $img,
                "class" => $class
            ));
    ...
    case 'showall';
        ...
        if (_rows($qry)) {
            ...
            while ($get = _fetch($qry)) {
                ...
                $gegner = show(_cw_details_gegner, array(
                    "gegner" => re(cut($get['clantag'] . " - " . $get['gegner'], $lcwgegner)),
                ...
                $icon  = show(_gameicon, array(
                    "icon" => $get['icon']
                ));
                
                $show .= show($dir . "/clanwars_show", array(
            ...
            if (_rows($qry)) {
                ...
                $anz_spiele_squads = show(_cw_stats_spiele_squads, array(
                    "anz_squads" => $anz_squads,
                    "anz_games" => $anz_games
                ));
                if ($anz_wo_wars != "0")
                    $wo_balken = show(_votes_balken, array(
            ...
            $show = show($dir . "/clanwars_no_show", array(
                "clanwars_no_show" => _clanwars_no_show
            ));
        }
        
        $nav  = nav($entrys, $maxcw, "?action=showall&amp;id=" . $_GET['id'] . "");
        $show = show($dir . "/clanwars", array(
            "head" => _cw_head_clanwars,
            "game" => _cw_head_game,
            "datum" => _cw_head_datum,
            "liga" => _cw_head_liga,
            "gametype" => _cw_head_gametype,
        ...
        while ($get = _fetch($qry)) {
            ...
            $color++;
            $img = squad($get['icon']);
            $legende .= show(_awards_legende, array(
                "game" => re($get['game']),
                "img" => $img,
                "class" => $class
            ));
        ...
        $legende = show($dir . "/legende", array(
            "legende_head" => _awards_head_legende,
            "legende" => $legende
        ));
        
        $index = show($dir . "/main", array(
            "head" => _awards_head,
            "stats" => $stats,
            "legende" => $legende,
            "show" => $show
    ...
    case 'details';
        ...
        if ($chkMe != "1" && $chkMe != "unlogged" && $get['punkte'] == "0" && $get['gpunkte'] == "0") {
            if ($get['datum'] > time()) {
                ...
                while ($getp = _fetch($qryp)) {
                    ...
                    $class = ($color % 2) ? "contentMainSecond" : "contentMainFirst";
                    $color++;
                    $show_players .= show($dir . "/players_show", array(
                        "nick" => autor($getp['member']),
                        "class" => $class,
                        "status" => $status
                    ));
                }
                ...
                $players = show($dir . "/players", array(
                ...
                $serverpwd = show(_cw_serverpwd, array(
        ...
        $img    = squad($get['icon']);
        $show   = show(_cw_details_squad, array(
            "game" => re($get['game']),
            "name" => re($get['name']),
            "id" => $get['squad_id'],
            "img" => $img
        ...
        $flagge = flag($get['gcountry']);
        $gegner = show(_cw_details_gegner_blank, array(
            "gegner" => re($get['clantag'] . " - " . $get['gegner']),
            "url" => !empty($get['url']) ? re($get['url']) : "#"
        ));
        $server = show(_cw_details_server, array(
            "servername" => re($get['servername']),
        ...
        if (permission("clanwars")) {
            $editcw = show("page/button_edit_single", array(
                "id" => $get['id'],
        ...
        if (!empty($screen1) || !empty($screen2) || !empty($screen3) || !empty($screen4)) {
            $screens = show($dir . "/screenshots", array(
                "head" => _cw_screens,
        ...
        $comments = show($dir . "/comments", array(
            "head" => _cw_comments_head,
            "show" => $comments,
            "seiten" => $seiten,
            "add" => $add
        ));
        ...
        $index = show($dir . "/details", array(
            "head" => _cw_head_details,
    ...
    case 'preview';
        ...
        $serverpwd = show(_cw_serverpwd, array(
            "cw_serverpwd" => re($_POST['serverpwd'])
        ));
        ...
        $flagge = flag($get['gcountry']);
        $gegner = show(_cw_details_gegner_blank, array(
            "gegner" => re($_POST['clantag'] . " - " . $_POST['gegner']),
            "url" => links($_POST['url'])
        ));
        $server = show(_cw_details_server, array(
            "servername" => re($_POST['servername']),
        ...
        if (!empty($_POST['s2']))
            $screen2 = '<img src="../inc/images/admin/cwscreen.jpg" alt="" />';
        else
            $screen2 = "";
        
        if (!empty($_POST['s3']))
            $screen3 = '<img src="../inc/images/admin/cwscreen.jpg" alt="" />';
        else
            $screen3 = "";
        
        if (!empty($_POST['s4']))
            $screen4 = '<img src="../inc/images/admin/cwscreen.jpg" alt="" />';
        else
            $screen4 = "";
        ...
        if (empty($_POST['xonx1']) && empty($_POST['xonx2']))
            ...
            $xonx = $_POST['xonx1'] . "on" . $_POST['xonx2'];
