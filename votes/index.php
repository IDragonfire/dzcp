...
switch ($action):
    default:
        ...
        $qry = db('SELECT * FROM ' . $db['votes'] . $whereIntern . $fvote . ' ORDER BY ' . $orderIntern . ' datum DESC');
        while ($get = _fetch($qry)) {
            $qryv    = db('SELECT * FROM ' . $db['vote_results'] . ' WHERE vid = ' . (int) $get['id'] . ' ORDER BY id');
            $results = '';
            $check   = '';
            $stimmen = 0;
            $vid     = 'vid_' . (int) $get['id'];
            if ($get['intern'] == 1) {
                $showVoted = '';
                $check     = db('SELECT * FROM ' . $db['ipcheck'] . ' WHERE what = "' . $vid . '" AND ip = ' . (int) $userid . '');
            ...
            while ($getv = _fetch($qryv)) {
                ...
                if ($hostIpcheck || $ipcheck || isset($_COOKIE[$prev . "vid_" . $get['id']]) || $get['closed'] == 1) {
                    ...
                    $votebutton  = "";
                    $results .= show($dir . "/votes_results", array(
                        "answer" => re($getv['sel']),
                        "percent" => $percent,
                    ...
                    $results .= show($dir . "/votes_vote", array(
                        "id" => $getv['id'],
                        "answer" => re($getv['sel']),
                        "class" => $class
                    ));
            ...
            if ($get['intern'] == 1 && $stimmen != 0 && ($get['von'] == $userid || permission('votes'))) {
                $showVoted = ' <a href="?action=showvote&amp;id=' . (int) $get['id'] . '"><img src="../inc/images/lupe.gif" alt="" title="' . _show_who_voted . '" class="icon" /></a>';
            }
            ...
            if ($get['forum'] == 1)
                $ftitel = re($get['titel']) . ' (Forum)';
            else
                $ftitel = re($get['titel']);
            
            $titel = show(_votes_titel, array(
                "titel" => $ftitel,
                "vid" => $get['id'],
                "icon" => $moreicon,
                "intern" => $intern
            ...
            if ($get['closed'] == 1)
                $closed = _closedicon_votes;
            else
                $closed = "";
            
            $class = ($color2 % 2) ? "contentMainSecond" : "contentMainFirst";
            $color2++;
            $show .= show($dir . "/votes_show", array(
    ...
    case 'showvote';
        ...
        if ($get['intern'] == 1) {
            ...
            while ($getv = _fetch($qryv)) {
                $class = ($color % 2) ? "contentMainSecond" : "contentMainFirst";
                $color++;
                $show .= show($dir . "/voted_show", array(
                    "user" => autor($getv['ip']),
