    ...
    default:
        ...
        if (_rows($qry)) {
            while ($get = _fetch($qry)) {
                ...
                $show .= show($dir . "/artikel_show", array(
                    "titel" => $titel,
                    "kat" => $kat,
                    "id" => $get['id'],
                    "display" => "none",
    ...
    case 'show';
        ...
        if (_rows($qry) == 0) {
            ...
            while ($get = _fetch($qry)) {
                ...
                if ($get['url1']) {
                    $rel    = _related_links;
                    $links1 = show(_artikel_link, array(
                        "link" => re($get['link1']),
                        "url" => $get['url1']
                    ));
                } else {
                ...
                if ($get['url2']) {
                    $rel    = _related_links;
                    $links2 = show(_artikel_link, array(
                        "link" => re($get['link2']),
                        "url" => $get['url2']
                    ));
                } else {
                ...
                if ($get['url3']) {
                    $rel    = _related_links;
                    $links3 = show(_artikel_link, array(
                        "link" => re($get['link3']),
                        "url" => $get['url3']
                    ));
                } else {
                ...
                if (!empty($links1) || !empty($links2) || !empty($links3)) {
                    $links = show(_artikel_links, array(
                        "link1" => $links1,
                        "link2" => $links2,
                        "link3" => $links3,
                        "rel" => $rel
                ...
                if (settings("reg_artikel") == "1" && $chkMe == "unlogged") {
                    ...
                    if (!ipcheck("artid(" . $_GET['id'] . ")", $flood_newscom)) {
                        $add = show("page/comments_add", array(
                            "titel" => _artikel_comments_write_head,
                            "bbcodehead" => _bbcode,
                            "form" => $form,
                            "show" => "none",
                            "b1" => $u_b1,
                ...
                $index = show($dir . "/show_more", array(
                    "titel" => re($get['titel']),
                    "id" => $get['id'],
                    "comments" => "",
                    "display" => "inline",
