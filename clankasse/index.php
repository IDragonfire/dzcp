    ...
    default:
        if ($chkMe == "unlogged" || $chkMe < "2") {
            ...
            while ($get = _fetch($qry)) {
                ...
                if ($get['pm'] == "0") {
                    $pm = show(_clankasse_plus, array(
                        "betrag" => $betrag,
                        "w" => $w
                    ));
                } else {
                    $pm = show(_clankasse_minus, array(
                        "betrag" => $betrag,
                        "w" => $w
                    ));
                }
                ...
                $show .= show($dir . "/clankasse_show", array(
                    "betrag" => $pm,
                    "id" => $get['id'],
                    "class" => $class,
                    "for" => re($get['member']),
            ...
            if ($getp['gesamt'] < $getc['gesamt']) {
                $gesamt = show(_clankasse_summe_minus, array(
                    "summe" => $ges,
                    "w" => $w
                ));
            } else {
                $gesamt = show(_clankasse_summe_plus, array(
                    "summe" => $ges,
                    "w" => $w
                ));
            }
            ...
            while ($gets = _fetch($qrys)) {
                if ($gets['user']) {
                    if (paycheck($gets['payed'])) {
                        $status = show(_clankasse_status_payed, array(
                            "payed" => date("d.m.Y", $gets['payed'])
                        ));
                    } elseif (date("d.m.Y", $gets['payed']) == date("d.m.Y", time())) {
                        $status = show(_clankasse_status_today, array());
                    } else {
                        $status = show(_clankasse_status_notpayed, array(
                            "payed" => date("d.m.Y", $gets['payed'])
                        ));
                    }
                } else {
                ...
                if (permission("clankasse"))
                    $edit = show(_admin_ck_edit, array(
                        "id" => $gets['id']
                    ));
                $class = ($color % 2) ? "contentMainSecond" : "contentMainFirst";
                $color++;
                
                $showstatus .= show($dir . "/status", array(
                    "nick" => autor($gets['id']),
                    "status" => $status,
                    "class" => $class,
                    "edit" => $edit
            ...
            $index  = show($dir . "/clankasse", array(
                "show" => $show,
    ...
    case 'admin':
        if (permission("clankasse")) {
            if ($_GET['do'] == "new") {
                ...
                while ($get = _fetch($qry)) {
                    $trans .= show(_select_field, array(
                        "value" => re($get['kat']),
                        "sel" => "",
                        "what" => re($get['kat'])
                    ));
