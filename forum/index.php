...
switch ($action):
    default:
        ...
        while ($get = _fetch($qry)) {
            ...
            while ($gets = _fetch($qrys)) {
                if ($get['intern'] == 0 || ($get['intern'] == 1 && fintern($gets['id']))) {
                    ...
                    if (cnt($db['f_threads'], " WHERE kid = '" . $gets['id'] . "'") == "0") {
                        ...
                        $lpost .= show(_forum_thread_lpost, array(
                            "nick" => autor($getlt['t_reg'], '', $getlt['t_nick'], $getlt['t_email']),
                        ...
                        $lpost .= show(_forum_thread_lpost, array(
                            "nick" => autor($getlp['reg'], '', $getlp['nick'], $getlp['email']),
                    ...
                    $showt .= show($dir . "/kats_show", array(
                        "topic" => re($gets['kattopic']),
            ...
            if ($get['intern'] == 1)
                $katname = show(_forum_katname_intern, array(
                    "katname" => re($get['name'])
                ));
            else
                $katname = re($get['name']);
        ...
        $threads = show(_forum_cnt_threads, array(
            "threads" => cnt($db['f_threads'])
        ));
        $posts   = show(_forum_cnt_posts, array(
            "posts" => cnt($db['f_posts']) + cnt($db['f_threads'])
        ));
        ...
        while ($gettp = _fetch($qrytp)) {
            $class = ($color % 2) ? "contentMainSecond" : "contentMainFirst";
            $color++;
            $show_top .= show($dir . "/top_posts_show", array(
                "nick" => autor($gettp['user']),
        ...
        $top_posts = show($dir . "/top_posts", array(
            "head" => _forum_top_posts,
            "show" => $show_top,
            "nick" => _nick,
            "posts" => _forum_posts
        ));
        ...
        if (_rows($qryo)) {
            ...
            if ($chkMe == "unlogged")
                $nick = "<center>" . _forum_nobody_is_online . "</center>";
            else
                $nick = "<center>" . _forum_nobody_is_online2 . "</center>";
        }
        
        $online = show($dir . "/online", array(
            "nick" => $nick,
            "head" => _forum_online_head
        ));
        
        $index = show($dir . "/forum", array(
            "head" => _forum_head,
            "threads" => $threads,
    ...
    case 'show';
        ...
        if ($checks['intern'] == 1 && (!permission("intforum") && !fintern($checks['id']))) {
            ...
            while ($get = _fetch($qry)) {
                if ($get['sticky'] == "1")
                    $sticky = _forum_sticky;
                else
                    $sticky = "";
                
                if ($get['global'] == "1")
                    $global = _forum_global;
                else
                    $global = "";
                
                if ($get['closed'] == "1")
                    $closed = show("page/button_closed", array());
                else
                    $closed = "";
                ...
                if (empty($_POST['suche'])) {
                    ...
                    $threadlink = show(_forum_thread_link, array(
                        "topic" => re(cut($get['topic'], $lforumtopic)),
                        "id" => $get['id'],
                        "kid" => $gets['id'],
                        "sticky" => $sticky,
                        "global" => $global,
                ...
                $class = ($color % 2) ? "contentMainSecond" : "contentMainFirst";
                $color++;
                $threads .= show($dir . "/forum_show_threads", array(
                    "new" => check_new($get['lp']),
                    "topic" => $threadlink,
            ...
            $search = show($dir . "/forum_skat_search", array(
                "head_search" => _forum_head_skat_search,
                "id" => $_GET['id'],
            ...
            if (!empty($_POST['suche'])) {
                $what = show($dir . "/search", array(
                    "head" => _forum_search_head,
                ...
                $new  = show(_forum_new_thread, array(
                    "id" => $_GET['id']
                ));
            ...
            $wheres = show(_forum_subkat_where, array(
                "where" => re($gets['kattopic']),
                "id" => $gets['id']
            ));
            
            $index = show($dir . "/forum_show", array(
                "head" => _forum_head,
                "where" => $wheres,
                "mainkat" => re($kat['name']),
                "what" => $what,
    ...
    case 'showthread';
        ...
        if (_rows($f_check)) {
            if ($checks['intern'] == 1 && !permission("intforum") && !fintern($checks['id'])) {
                ...
                $lpost = show(_forum_lastpost, array(
                    "id" => $entrys + 1,
                    "tid" => $_GET['id'],
                    "page" => $pagenr . $hL
                ));
                ...
                while ($getp = _fetch($qryp)) {
                    if (data($getp['reg'], "signatur"))
                        $sig = _sig . bbcode(data($getp['reg'], "signatur"));
                    else
                        $sig = "";
                    
                    if ($getp['reg'] != 0)
                        $userposts = show(_forum_user_posts, array(
                            "posts" => userstats($getp['reg'], "forumposts")
                        ));
                    else
                        $userposts = "";
                    ...
                    $zitat = show("page/button_zitat", array(
                        "id" => $_GET['id'],
                    ...
                    $titel = show(_eintrag_titel_forum, array(
                        "postid" => $i + ($page - 1) * $maxfposts,
                    ...
                    if ($getp['reg'] != 0) {
                      ...
                      WHERE id = '" . $getp['reg'] . "'");
                        ...
                        $email = show(_emailicon_forum, array(
                            "email" => eMailAddr($getu['email'])
                        ));
                        $pn    = show(_pn_write_forum, array(
                            "id" => $getp['reg'],
                            "nick" => $getu['nick']
                        ));
                        ...
                        if (empty($getu['icq']) || $getu['icq'] == 0)
                            ...
                            $uin = show(_icqstatus_forum, array(
                                "uin" => $getu['icq']
                            ));
                    ...
                    $show .= show($dir . "/forum_posts_show", array(
                        "nick" => $nick,
                ...
                $wheres = show(_forum_post_where, array(
                    "wherepost" => re($getw['topic']),
                ...
                if ($get['t_reg'] == "0") {
                    ...
                    $userposts = show(_forum_user_posts, array(
                        "posts" => userstats($get['t_reg'], "forumposts")
                    ));
                }
                ...
                if (data($get['t_reg'], "signatur"))
                    $sig = _sig . bbcode(data($get['t_reg'], "signatur"));
                else
                    $sig = "";
                
                if ($get['t_reg'] == $userid || permission("forum"))
                    $editt = show("page/button_edit_single", array(
                        "id" => $get['id'],
                ...
                if (permission("forum")) {
                    ...
                    while ($getok = _fetch($qryok)) {
                      ...
                      ORDER BY kattopic");
                        while ($geto = _fetch($qryo)) {
                            $skat .= show(_forum_select_field_skat, array(
                                "value" => $geto['id'],
                                "what" => re($geto['kattopic'])
                            ));
                        }
                        
                        $move .= show(_forum_select_field_kat, array(
                            "value" => "lazy",
                            "what" => re($getok['name']),
                            "skat" => $skat
                        ));
                ...
                if ($chkMe == "unlogged") {
                    ...
                    $f_abo = show($dir . "/forum_abo", array(
                        "id" => intval($_GET['id']),
                        "abo" => $abo,
        ...
        if ($_GET['do'] == "edit") {
            ...
            if ($get['t_reg'] == $userid || permission("forum")) {
                if (permission("forum")) {
                    ...
                    $admin = show($dir . "/form_admin", array(
                        "adminhead" => _forum_admin_head,
                ...
                if (empty($get['vote'])) {
                    $vote = show($dir . "/form_vote", array(
                        "head" => _votes_admin_head,
                ...
                $dowhat = show(_forum_dowhat_edit_thread, array(
                    "id" => $_GET['id']
                ));
                $index  = show($dir . "/thread", array(
                    "titel" => _forum_edit_thread_head,
                    "nickhead" => _nick,
                   ...
                   WHERE id = '" . intval($_GET['id']) . "'");
                    ...
                    while ($getabo = _fetch($checkabo)) {
                        if ($userid != $getabo['user']) {
                            ...
                            $subj = show(settings('eml_fabo_tedit_subj'), array(
                                "titel" => $title
                            ));
                            
                            $message = show(settings('eml_fabo_tedit'), array(
                                "nick" => re($getabo['nick']),
            ...
            if (settings("reg_forum") == "1" && $chkMe == "unlogged") {
                ...
                if (!ipcheck("fid(" . $_GET['kid'] . ")", $flood_forum)) {
                    ...
                    $dowhat = show(_forum_dowhat_add_thread, array(
                        "kid" => $_GET['kid']
                    ));
                    
                    $index = show($dir . "/thread", array(
                        "titel" => _forum_new_thread_head,
                        "nickhead" => _nick,
                        "topichead" => _forum_topic,
                        "subtopichead" => _forum_subtopic,
                        "emailhead" => _email,
                ...
                if (settings("reg_forum") == "1" && $chkMe == "unlogged") {
                    ...
                    if ($toCheck) {
                        ...
                        if (!empty($_POST['question'])) {
                            ...
                            if ($fgetvote['intern'] == 1)
                                $ivote = "`intern` = '1',";
                            else
                                $ivote = "`intern` = '" . ((int) $_POST['intern']) . "',";
            ...
            if ($get['reg'] == $userid || permission("forum")) {
                ...
                if ($toCheck) {
                    if ($get['reg'] != 0) {
                        ...
                        if (($_POST['secure'] != $_SESSION['sec_' . $dir]) && !isset($userid))
                            $error = _error_invalid_regcode;
                        elseif (empty($_POST['nick']))
                            $error = _empty_nick;
                        elseif (empty($_POST['email']))
                   ...
                   WHERE id = '" . intval($_GET['id']) . "'");
                    ...
                    while ($getabo = _fetch($checkabo)) {
                        if ($userid != $getabo['user']) {
                            ...
                            $subj = show(settings('eml_fabo_pedit_subj'), array(
                                "titel" => $title
                            ));
                            
                            $message = show(settings('eml_fabo_pedit'), array(
                                "nick" => re($getabo['nick']),
                                "postuser" => fabo_autor($userid),
                                "topic" => $gettopic['topic'],
                                "titel" => $title,
                    ...
                    $lpost = show(_forum_add_lastpost, array(
                        "id" => $entrys + 1,
                        "tid" => $getp['sid'],
                        "page" => $pagenr
                    ));
            ...
            if (settings("reg_forum") == "1" && $chkMe == "unlogged") {
                ...
                if (!ipcheck("fid(" . $_GET['kid'] . ")", $flood_forum)) {
                    ...
                    if (forumcheck($_GET['id'], "closed")) {
                        ...
                        $dowhat = show(_forum_dowhat_add_post, array(
                            "id" => $_GET['id'],
                            "kid" => $_GET['kid']
                        ));
                      ...
                      ORDER BY date DESC");
                        if (_rows($qryl)) {
                            ...
                            if (data($getl['reg'], "signatur"))
                                $sig = _sig . bbcode(data($getl['reg'], "signatur"));
                            else
                                $sig = "";
                            
                            if ($getl['reg'] != "0")
                                $userposts = show(_forum_user_posts, array(
                                    "posts" => userstats($getl['reg'], "forumposts")
                                ));
                            else
                                $userposts = "";
                            ...
                            $titel = show(_eintrag_titel_forum, array(
                                "postid" => (cnt($db['f_posts'], " WHERE sid =" . intval($_GET['id'])) + 1),
                          ...
                          WHERE id = '" . $getl['reg'] . "'");
                            ...
                            $lastpost = show($dir . "/forum_posts_show", array(
                                "nick" => cleanautor($getl['reg'], '', $getl['nick'], $getl['email']),
                                "postnr" => "",
                                "text" => $text,
                        ...
                        AND id = '" . intval($_GET['id']) . "'");
                            ...
                            if ($gett['t_reg'] != "0")
                                $userposts = show(_forum_user_posts, array(
                                    "posts" => userstats($gett['t_reg'], "forumposts")
                                ));
                            else
                                $userposts = "";
                          ...
                          WHERE id = '" . $gett['t_reg'] . "'");
                            ...
                            $lastpost = show($dir . "/forum_posts_show", array(
                                "nick" => cleanautor($gett['t_reg'], '', $gett['t_nick'], $gett['t_email']),
                                "postnr" => "",
                                "text" => $text,
                ...
                if (settings("reg_forum") == "1" && $chkMe == "unlogged") {
                    ...
                    if ($toCheck) {
                        ...
                        if (_rows($qrydp)) {
                            ...
                            if (isset($userid)) {
                                if ($userid == $gettdp['t_reg'] && $double_post == 1)
                                    $spam = 2;
                                else
                                    $spam = 0;
                            } else {
                                if ($_POST['nick'] == $gettdp['t_nick'] && $double_post == 1)
                                    $spam = 2;
                                else
                                    $spam = 0;
                            }
                        ...
                        if ($spam == 1) {
                            ...
                            $text = show(_forum_spam_text, array(
                                "autor" => $fautor,
                        ...
                        while ($getabo = _fetch($checkabo)) {
                            if ($userid != $getabo['user']) {
                                ...
                                $subj = show(settings('eml_fabo_npost_subj'), array(
                                    "titel" => $title
                                ));
                                
                                $message = show(settings('eml_fabo_npost'), array(
                                    "nick" => re($getabo['nick']),
                                    "postuser" => fabo_autor($userid),
                                    "topic" => $gettopic['topic'],
                                    "titel" => $title,
                ...
                if (isset($_POST['delete'])) {
                   ...
                   WHERE id = '" . intval($_GET['id']) . "'");
                    ...
                    foreach ($userPostReduction as $key_id => $value_postDecrement) {
                        db('UPDATE ' . $db['userstats'] . ' SET `forumposts` = `forumposts` - ' . $value_postDecrement . ' WHERE user = ' . $key_id);
                    }
                    ...
                    if ($_POST['move'] == "lazy") {
                      ...
                      WHERE s1.id = '" . intval($_GET['id']) . "'");
                        ...
                        $i_move = show(_forum_admin_do_move, array(
                            "kat" => re($getm['kattopic'])
                        ));
    ...
    case 'preview';
        ...
        if ($_GET['what'] == 'thread') {
            ...
            if ($guestCheck) {
                ...
                $userposts = show(_forum_user_posts, array(
                    "posts" => userstats($pUId, "forumposts") + 1
                ));
            } else {
                $pn    = "";
                $icq   = "";
                $email = show(_emailicon_forum, array(
                    "email" => eMailAddr($_POST['email'])
                ));
                if (empty($_POST['hp']))
                    $hp = "";
                else
            ...
            $wheres = show(_forum_post_where_preview, array(
                "wherepost" => re($_POST['topic']),
                "wherekat" => re($getw['kattopic']),
                "mainkat" => re($kat['name']),
                "tid" => $_GET['id'],
                "kid" => $getw['kid']
