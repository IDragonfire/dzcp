...
if ($gametiger_game == "lazy")
    $gametiger_game = "all";
elseif ($gametiger_game == "bf1942")
    $gametiger_game = "bf";
    ...
    default:
        ...
        if (is_array($player) && count($player) > 0) {
            for ($i = 0; $i < count($player); $i++) {
                ...
                $color++;
                $gametiger .= show(_gametiger_player_results, array(
                    "players" => $player[$i],
                    "servers" => $server[$i],
                    "class" => $class
                ));
        ...
        $gsearch = show($dir . "/footer", array(
            "gametiger" => $gametiger,
            "id" => count($player),
            "found" => _gt_found
        ));
        
        $search = show($dir . "/psearch", array(
            "show" => $gsearch,
        ...
        $index = show($dir . "/gametiger", array(
            "show" => $search,
    ...
    case 'server':
        ...
        if (is_array($serverip) && count($serverip) > 0) {
            for ($i = 0; $i < count($serverip); $i++) {
                $class = ($color % 2) ? "contentMainSecond" : "contentMainFirst";
                $color++;
                $gametiger .= show(_gametiger_server_results, array(
    ...
    case 'map':
        ...
        if (is_array($aktmap) && count($aktmap) > 0) {
            for ($i = 0; $i < count($aktmap); $i++) {
                $class = ($color % 2) ? "contentMainSecond" : "contentMainFirst";
                $color++;
                $gametiger .= show(_gametiger_map_results, array(
                    "name" => $name[$i],
                    "ip" => $sip[$i],
                    "class" => $class,
                    "aktmap" => $aktmap[$i],
